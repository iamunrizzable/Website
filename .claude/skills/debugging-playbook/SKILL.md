...
Order: ① score actually ≥ 60? (`POST /api/moderate` the exact text to check) ② `ICLOUD_EMAIL_ADDRESS`/`ICLOUD_APP_PASSWORD` set? (unset = silent skip, only a console.warn) ③ iCloud app-specific password valid / not rate-limited? ④ Recipient is hardcoded `tyler@tjbmanagementinc.com` in `lib/email/alerts.js`.
