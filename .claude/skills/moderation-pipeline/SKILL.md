...
## Per-operator category filters ("Comment Filters" panel) — added July 2026

Operators can now toggle which of the 8 non-mandatory scorer categories actually get applied when hiding comments — e.g. a creator doing spicy content might disable `profanity` while keeping `hate_speech`/`harassment`/`scam` on. `potential_minor` is excluded from this toggle set and ALWAYS scores regardless of the operator's selection — it's tied to the block-queue/email-alert pipeline (child safety), not comment cleanliness, and disabling it was explicitly ruled out.

`scoreContent(text, customRules, enabledCategories)` takes a third optional argument: `null` (default, used everywhere that hasn't been updated, e.g. `/api/moderate`) means "no filter configured — every category applies," matching pre-existing behavior exactly. An array means "only these categories, plus potential_minor, are scored" — categories not in the array are skipped entirely (no points, no flag), including the `caps` modifier. `TOGGLEABLE_CATEGORIES` and `CATEGORY_META` (label + description per category) are exported from `lib/moderation/scorer.js` as the single source of truth — both the admin and system API routes and UI panels read from it instead of duplicating category copy.

Storage mirrors the custom-rules pattern exactly: admin's selection lives in `lib/tokens.js` (`getCategoryFilters`/`storeCategoryFilters`, Redis-or-memory, key `moderation:category_filters`, returns `null` when never configured) and is read once per run in `lib/sync.js`. System's selection lives in a `category_filters` HttpOnly cookie (per-operator isolation, same as `custom_rules`) and is read once per run in `app/api/system/sync/route.js`. Both are read via new `GET`/`POST /api/business/category-filters` (x-admin-key) and `/api/system/category-filters` (cookie) routes — GET returns `{ enabled, categories }` (defaulting `enabled` to all `TOGGLEABLE_CATEGORIES` when nothing's been saved yet), POST saves a new `enabled` array. Like custom rules, filter changes only take effect on the next Comment Sync run — no real-time hook. The mirrored "Comment Filters" panel on both `/admin` and `/system` renders one checkbox per category from the API's `categories` array, plus a locked, always-checked "Potential Minor Detection — Always On" row for transparency.
