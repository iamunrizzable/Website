...
| potential_minor | 70 | age statements, grade levels, "minor"/"underage" — ALSO queues an account block when enabled |
...
Score capped at 100. **Thresholds: ≥ 25 → `action: 'hide'`; ≥ 60 → email alert (`shouldAlert`).** Note 25 means a single spam or promo+caps match triggers hiding — when Tyler says "it's hiding too much / too little," first check whether the operator has disabled that category in their Comment Filters (see below) before touching pattern lists or these two numbers.
...
## Per-operator category filters ("Comment Filters" panel) — added July 2026, potential_minor made toggleable September 2026

Operators can toggle which of the 9 scorer categories actually get applied when hiding comments — e.g. a creator doing spicy content might disable `profanity` while keeping `hate_speech`/`harassment`/`scam` on. `potential_minor` was originally locked always-on (child-safety rationale), but Tyler explicitly asked for a real off switch and confirmed "everything off" — comment hiding AND the block-queue/email-alert pipeline — when the tradeoff was flagged to him. It is now a normal entry in `TOGGLEABLE_CATEGORIES`/`CATEGORY_META`, no special-casing anywhere in `scoreContent`.

`scoreContent(text, customRules, enabledCategories)` takes a third optional argument: `null` (default, used everywhere that hasn't been updated, e.g. `/api/moderate`) means "no filter configured — every category applies, including potential_minor," matching pre-existing behavior exactly for anyone who's never touched Comment Filters. An array means "only these categories are scored" — categories not in the array (potential_minor included) are skipped entirely: no points, no flag, including the `caps` modifier. Since `lib/sync.js`'s block-queue (`flags.includes('potential_minor')`) and email alert (`shouldAlert(score)`) are both driven off that same flag/score, disabling the category there naturally disables both downstream effects too — no separate wiring needed. `TOGGLEABLE_CATEGORIES` and `CATEGORY_META` (label + description per category) are exported from `lib/moderation/scorer.js` as the single source of truth — both the admin and system API routes and UI panels read from it instead of duplicating category copy.

Storage mirrors the custom-rules pattern exactly: admin's selection lives in `lib/tokens.js` (`getCategoryFilters`/`storeCategoryFilters`, Redis-or-memory, key `moderation:category_filters`, returns `null` when never configured) and is read once per run in `lib/sync.js`. System's selection lives in a `category_filters` HttpOnly cookie (per-operator isolation, same as `custom_rules`) and is read once per run in `app/api/system/sync/route.js`. Both are read via new `GET`/`POST /api/business/category-filters` (x-admin-key) and `/api/system/category-filters` (cookie) routes — GET returns `{ enabled, categories }` (defaulting `enabled` to all `TOGGLEABLE_CATEGORIES` when nothing's been saved yet), POST saves a new `enabled` array. Like custom rules, filter changes only take effect on the next Comment Sync run — no real-time hook. The mirrored "Comment Filters" panel on both `/admin` and `/system` renders one plain checkbox per category from the API's `categories` array — no locked/disabled row anymore.
...
Nodemailer → SMTP (iCloud Mail only, not Gmail — `smtp.mail.me.com` default) → tyler@tjbmanagementinc.com, hardcoded. Skips silently when `ICLOUD_EMAIL_ADDRESS`/`ICLOUD_APP_PASSWORD` unset — so "no alert arrived" first check is env vars, second is score < 60. Minor detections get a special 🚨 subject + red banner.
