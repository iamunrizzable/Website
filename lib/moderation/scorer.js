...
// Category metadata for the operator-facing "Comment Filters" toggle panel.
// potential_minor is deliberately excluded — it's mandatory (see scoreContent)
// because disabling it would also disable the block-queue/email-alert pipeline
// for suspected minor accounts, not just comment cleanliness.
export const CATEGORY_META = {
  hate_speech: { label: 'Hate Speech', description: 'Slurs and hateful language' },
  harassment: { label: 'Harassment', description: 'Death wishes, "kys", targeted personal attacks' },
  scam: { label: 'Scams & Phishing', description: 'Links, crypto, "DM me to earn", fake recovery-account bait' },
  negativity: { label: 'Negativity & Mockery', description: 'Insults, "L + ratio", dismissive put-downs' },
  profanity: { label: 'Profanity', description: 'Swearing and obfuscated variants (f*ck, sh1t, etc.)' },
  spam: { label: 'Spam', description: 'Follow-for-follow, bio/link bait, repeated characters' },
  promo: { label: 'Self-Promotion', description: '"Check out my page", "subscribe to my channel"' },
  caps: { label: 'Excessive Caps', description: 'Comments that are mostly uppercase' },
};

export const TOGGLEABLE_CATEGORIES = Object.keys(CATEGORY_META);

// Always scored regardless of an operator's filter selection — tied to
// child-safety detection and the block-queue/email-alert pipeline.
const MANDATORY_CATEGORIES = ['potential_minor'];

export function scoreContent(text, customRules = [], enabledCategories = null) {
  if (!text || typeof text !== 'string') {
    return { score: 0, flags: [], action: 'allow' };
  }

  // null/undefined = no filter configured yet -> every category applies
  // (preserves pre-existing behavior for operators who haven't set filters).
  const enabledSet = enabledCategories === null
    ? null
    : new Set([...enabledCategories, ...MANDATORY_CATEGORIES]);

  let score = 0;
  const flags = [];

  for (const [category, { score: pts, patterns }] of Object.entries(RULES)) {
    if (enabledSet && !enabledSet.has(category)) continue;
  ...
  // Excessive ALL CAPS
  if ((!enabledSet || enabledSet.has('caps')) && text.length > 10 && (text.match(/[A-Z]/g) ?? []).length / text.length > 0.7) {
    flags.push('caps');
    score += 10;
  }
