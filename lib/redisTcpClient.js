...
// product, hostname *.db.redis.io), which only exposes a plain `REDIS_URL`
// TCP connection string, no REST API at all. Confirmed directly from the
// value shown in the Vercel dashboard, not guessed. @upstash/redis cannot
// use a `redis://` URL under any configuration, so this is a second,
// TCP-backed client — used ONLY when no REST credentials are configured
// (see getRedis() in lib/tokens.js) — that mimics the exact subset of the
// @upstash/redis API this codebase already calls, so none of those call
...
export function getTcpRedisAdapter() {
  return {
    async get(key) {
