...
export async function hideComment({ commentId, isHidden = true, videoId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/hide/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: actionBody({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId, is_hidden: isHidden }),
  });
  return parseRes(res);
}
...
export async function replyToComment({ videoId, commentId, content }) {
  ...
  const res = await fetch(`${BASE}/business/comment/reply/create/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: actionBody({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId, text: content }),
  });
  return parseRes(res);
}
...
export async function deleteComment({ commentId, videoId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/delete/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: actionBody({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId }),
  });
  return parseRes(res);
}
...
export async function pinComment({ commentId, isPinned = true, videoId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/pin/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: actionBody({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId, is_pinned: isPinned }),
  });
  return parseRes(res);
}
