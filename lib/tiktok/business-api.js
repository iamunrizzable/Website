...
async function commentAction(endpoint, payload) {
  const tokens = await acctAuth();
  const creatorId = tokens.business_id ?? tokens.open_id;
  const businessId = process.env.TIKTOK_ACCOUNT_BUSINESS_ID ?? creatorId;
  const body = actionBody({ business_id: businessId, creator_id: creatorId, ...payload });
