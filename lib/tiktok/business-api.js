...
export async function getAccount() {
  ...
  const fields = JSON.stringify(['display_name', 'profile_image', 'followers_count', 'likes_count', 'video_views_count', 'comment_count']);
  const params = new URLSearchParams({ business_id: tokens.advertiser_id, fields });
  const res = await fetch(`${BASE}/business/get/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}
...
export async function listVideos({ cursor = 0, pageSize = 20 } = {}) {
  const tokens = await auth();
  const fields = JSON.stringify(['video_id', 'title', 'create_time', 'statistics', 'status', 'thumbnail_url']);
  const params = new URLSearchParams({ business_id: tokens.advertiser_id, cursor, page_size: pageSize, fields });
  const res = await fetch(`${BASE}/business/video/list/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}
...
export async function listComments({ videoId, cursor = 0, pageSize = 20 }) {
  const tokens = await auth();
  const fields = JSON.stringify(['comment_id', 'text', 'create_time', 'username', 'like_count', 'status', 'parent_comment_id', 'is_pinned']);
  const params = new URLSearchParams({ business_id: tokens.advertiser_id, video_id: videoId, cursor, page_size: pageSize, fields });
  const res = await fetch(`${BASE}/business/comment/list/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function listCommentReplies({ commentId, cursor = 0, pageSize = 20 }) {
  const tokens = await auth();
  const params = new URLSearchParams({ business_id: tokens.advertiser_id, comment_id: commentId, cursor, page_size: pageSize });
  const res = await fetch(`${BASE}/business/comment/reply/list/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function hideComments({ commentIds, isHidden = true }) {
  const tokens = await auth();
  const res = await fetch(`${BASE}/business/comment/hide/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.advertiser_id, comment_ids: commentIds, is_hidden: isHidden }),
  });
  return parseRes(res);
}
...
export async function deleteComment({ commentId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/delete/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.advertiser_id, comment_id: commentId }),
  });
  return parseRes(res);
}

export async function likeComment({ commentId, isLiked = true }) {
  const tokens = await auth();
  const res = await fetch(`${BASE}/business/comment/like/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.advertiser_id, comment_id: commentId, is_liked: isLiked }),
  });
  return parseRes(res);
}
...
export async function pinComment({ commentId, isPinned = true }) {
  ...
  const res = await fetch(`${BASE}/business/comment/pin/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.advertiser_id, comment_id: commentId, is_pinned: isPinned }),
  });
  return parseRes(res);
}

export async function createComment({ videoId, content, commentId = null }) {
  const tokens = await auth();
  const body = { business_id: tokens.advertiser_id, video_id: videoId, content };
  if (commentId) body.comment_id = commentId;
  const res = await fetch(`${BASE}/business/comment/create/`, {
    method: 'POST',
    headers: hdrs(tokens),
...
export async function listMentionVideos({ cursor = 0, pageSize = 20 } = {}) {
  const tokens = await auth();
  const params = new URLSearchParams({ business_id: tokens.advertiser_id, cursor, page_size: pageSize });
...
export async function listTopMentionWords() {
  const tokens = await auth();
  const res = await fetch(`${BASE}/business/mention/top_word/list/?business_id=${tokens.advertiser_id}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function listTopMentionHashtags() {
  const tokens = await auth();
  const res = await fetch(`${BASE}/business/mention/top_hashtag/list/?business_id=${tokens.advertiser_id}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function listTrackedHashtags() {
  const tokens = await auth();
  const res = await fetch(`${BASE}/business/mention/hashtag/manage/list/?business_id=${tokens.advertiser_id}`, { headers: hdrs(tokens) });
  return parseRes(res);
}
...
export async function addTrackedHashtag({ hashtag }) {
  ...
  const res = await fetch(`${BASE}/business/mention/hashtag/add/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.advertiser_id, hashtag }),
  });
  return parseRes(res);
}
...
export async function getTrendingKeywords({ keyword = '' } = {}) {
  const tokens = await auth();
  const params = new URLSearchParams({ business_id: tokens.advertiser_id });
  if (keyword) params.set('keyword', keyword);
...
export async function getBenchmark() {
  const tokens = await auth();
  const res = await fetch(`${BASE}/business/benchmark/?business_id=${tokens.advertiser_id}`, { headers: hdrs(tokens) });
  return parseRes(res);
}
