...
// TikTok action endpoints expect video_id as int64 (unquoted JSON integer) but
// comment_id as a string. Build body as text and unquote only video_id values.
function actionBody(obj) {
  return JSON.stringify(obj).replace(/"video_id":"(\d{16,})"/g, '"video_id":$1');
}
...
export async function getAccount() {
  ...
  const params = new URLSearchParams({ business_id: businessId, fields: 'display_name,profile_image,followers_count,video_count' });
  ...
  return { ...info, avatar_url: info.profile_image ?? info.avatar_url, follower_count: info.followers_count ?? info.follower_count };
}
...
export async function listVideos({ cursor = 0, pageSize = 20 } = {}) {
  const tokens = await acctAuth();
  const businessId = tokens.business_id ?? tokens.open_id;
  const res = await fetch(`${BASE}/business/video/list/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({
      ...
      cursor,
      page_size: pageSize,
      fields: ['video_id', 'title', 'create_time', 'cover_image_url', 'view_count', 'like_count', 'comment_count', 'share_count'],
    }),
  });
  const data = await parseRes(res);
  // Normalize video_id → id so frontend key prop works
  if (data?.data?.videos) {
    data.data.videos = data.data.videos.map(v => ({ ...v, id: v.video_id ?? v.id }));
  }
  return data;
}
...
export async function hideComment({ commentId, isHidden = true, videoId }) {
  return commentAction('/business/comment/hide/', { video_id: videoId, comment_id: commentId, is_hidden: isHidden });
}
...
export async function deleteComment({ commentId, videoId }) {
  return commentAction('/business/comment/delete/', { video_id: videoId, comment_id: commentId });
}
