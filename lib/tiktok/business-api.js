...
// ── Comments ──────────────────────────────────────────────────────────────────
// Access-Token from advertiser token (has comment scope)
// business_id from account token (open_id)

export async function listComments({ videoId, cursor = 0, pageSize = 20 }) {
  const biz = await auth();
  const acct = await acctAuth();
  ...
  const res = await fetch(`${BASE}/business/comment/list/?${params}`, { headers: hdrs(biz) });
  return parseRes(res);
}

export async function hideComment({ commentId, isHidden = true }) {
  const biz = await auth();
  const acct = await acctAuth();
  const res = await fetch(`${BASE}/business/comment/hide/`, {
    method: 'POST',
    headers: hdrs(biz),
    body: JSON.stringify({ business_id: acct.business_id, comment_id: commentId, is_hidden: isHidden }),
  });
  return parseRes(res);
}
...
export async function replyToComment({ videoId, commentId, content }) {
  ...
  const res = await fetch(`${BASE}/business/comment/reply/create/`, {
    method: 'POST',
    headers: hdrs(biz),
    body: JSON.stringify({ business_id: acct.business_id, video_id: videoId, comment_id: commentId, content }),
  });
  return parseRes(res);
}
...
export async function deleteComment({ commentId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/delete/`, {
    method: 'POST',
    headers: hdrs(biz),
    body: JSON.stringify({ business_id: acct.business_id, comment_id: commentId }),
  });
  return parseRes(res);
}
...
export async function pinComment({ commentId, isPinned = true }) {
  ...
  const res = await fetch(`${BASE}/business/comment/pin/`, {
    method: 'POST',
    headers: hdrs(biz),
    body: JSON.stringify({ business_id: acct.business_id, comment_id: commentId, is_pinned: isPinned }),
  });
  return parseRes(res);
}
