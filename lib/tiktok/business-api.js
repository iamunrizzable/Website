...
// Uses /business/get/ (Business API), not the Open Platform /user/info/ —
// this is the endpoint that actually returns `username`, which several
// Mentions endpoints (e.g. hashtag/manage/list) require as a param.
export async function getAccount() {
  const tokens = await acctAuth();
  const businessId = tokens.business_id ?? tokens.open_id;
  const fields = encodeURIComponent(JSON.stringify(['display_name', 'profile_image', 'followers_count', 'likes', 'videos_count', 'username']));
  const res = await fetch(
    `${BASE}/business/get/?business_id=${encodeURIComponent(businessId)}&fields=${fields}`,
    { headers: hdrs(tokens) }
  );
...
// Unconfirmed request shape as of first attempt — `hashtag` (singular) is a
// best guess by analogy with the sibling hashtag/remove endpoint's confirmed
// field name. TikTok's own validation error will name the real param if
// this guess is wrong (see tiktok-api-calls skill); update this comment
// once confirmed live rather than re-guessing later.
export async function listMentionHashtagVideos({ hashtag, cursor = 0, pageSize = 20 } = {}) {
  const tokens = await acctAuth();
  const params = new URLSearchParams({ business_id: tokens.business_id, hashtag, cursor, page_size: pageSize });
  const res = await fetch(`${BASE}/business/mention/hashtag/video/list/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function listTrackedHashtags({ username }) {
  const tokens = await acctAuth();
  // TikTok requires a non-empty `username` param here (confirmed via
  // TikTok's own error: "username: value is required but missing"). Not
  // derivable from the stored token — the caller supplies it.
  const params = new URLSearchParams({ business_id: tokens.business_id, username });
  const res = await fetch(`${BASE}/business/mention/hashtag/manage/list/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function addTrackedHashtag({ hashtag, username }) {
  const tokens = await acctAuth();
  // Same required `username` param as listTrackedHashtags (confirmed via
  // TikTok's own error: "username: Missing data for required field.").
  ...
  const res = await fetch(`${BASE}/business/mention/hashtag/add/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.business_id, username, hashtags: [hashtag] }),
  });
  return parseRes(res);
}

// Confirmed against TikTok's own example request (Tyler pulled it
// directly from the portal docs): this is a LIST endpoint, not a
// validity check — it takes no hashtag param at all, just business_id
// + username, and returns { hashtag_list: [{ hashtag, create_date }] },
// the hashtags TikTok already considers verified/valid for that
// username. Passing a `hashtags` param (as this function used to) isn't
// part of the real request shape at all — that's almost certainly why
// every hashtag we tried came back "no valid hashtag for this
// username": TikTok was silently ignoring or choking on an unexpected
// param, not evaluating the hashtag we thought we were submitting.
export async function verifyHashtag({ username }) {
  const tokens = await acctAuth();
  const params = new URLSearchParams({ business_id: tokens.business_id, username });
  const res = await fetch(`${BASE}/business/mention/hashtag/verify/list/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

export async function removeTrackedHashtag({ hashtag, username }) {
  const tokens = await acctAuth();
  // NOT the same field as addTrackedHashtag — confirmed via a live error.
  // The mirrored assumption (hashtags, plural array) was wrong: TikTok
  // returned "hashtag: Missing data for required field." against that
  // request, meaning this endpoint takes a single `hashtag` string field.
  const res = await fetch(`${BASE}/business/mention/hashtag/remove/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.business_id, username, hashtag }),
  });
  return parseRes(res);
}
...
export async function getTrendingKeywords({ keyword }) {
  const tokens = await acctAuth();
  // This endpoint requires a non-empty `query` param (confirmed via
  // TikTok's own error: "query: value is required but missing" — the
  // param is named `query`, not `keyword`).
  const params = new URLSearchParams({ business_id: tokens.business_id, query: keyword });
...
export async function getBenchmark({ businessCategory }) {
  const tokens = await acctAuth();
  // TikTok requires a non-empty `business_category` param here (confirmed
  // via TikTok's own error: "business_category: value is required but
  // missing"). This is a TikTok-defined industry code we have no confirmed
  // enum list for — the caller supplies it.
  const params = new URLSearchParams({ business_id: tokens.business_id, business_category: businessCategory });
  const res = await fetch(`${BASE}/business/benchmark/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}
