...
// Diagnostic only (not a shipped feature): hashtag/add/ rejects every
// hashtag tried so far ("Invalid Params: some of hashtags are
// invalid.") regardless of case or content. Calls the one approved
// Mentions endpoint we've never used to see TikTok's real response
// before guessing at a fix.
// Same required `username` param as every other hashtag/* endpoint in
// this file (confirmed via TikTok's own error: "username: value is
// required but missing").
export async function verifyHashtag({ hashtag, username }) {
  const tokens = await acctAuth();
  const params = new URLSearchParams({ business_id: tokens.business_id, username, hashtags: JSON.stringify([hashtag]) });
