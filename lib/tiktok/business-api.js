...
// ── Comments (advertiser token for scope + open_id as business_id) ───────────
// The advertiser token has comment scopes; open_id from that token = business_id

export async function listComments({ videoId, cursor = 0, pageSize = 20 }) {
  const tokens = await auth();
  const params = new URLSearchParams({
    business_id: tokens.open_id,
    video_id: videoId,
    cursor,
    page_size: pageSize,
...
export async function hideComment({ commentId, isHidden = true }) {
  ...
  const res = await fetch(`${BASE}/business/comment/hide/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.open_id, comment_id: commentId, is_hidden: isHidden }),
  });
  return parseRes(res);
}
...
export async function replyToComment({ videoId, commentId, content }) {
  ...
  const res = await fetch(`${BASE}/business/comment/reply/create/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.open_id, video_id: videoId, comment_id: commentId, content }),
  });
  return parseRes(res);
}
...
export async function deleteComment({ commentId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/delete/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.open_id, comment_id: commentId }),
  });
  return parseRes(res);
}
...
export async function pinComment({ commentId, isPinned = true }) {
  ...
  const res = await fetch(`${BASE}/business/comment/pin/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: JSON.stringify({ business_id: tokens.open_id, comment_id: commentId, is_pinned: isPinned }),
  });
  return parseRes(res);
}
