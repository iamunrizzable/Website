...
// ── Account (requires TikTok account token) ───────────────────────────────────

export async function getAccount() {
  const tokens = await acctAuth();
  const fields = JSON.stringify(['display_name', 'profile_image', 'followers_count', 'total_likes', 'video_views', 'videos_count', 'username']);
  const params = new URLSearchParams({ business_id: tokens.business_id, fields });
  const res = await fetch(`${BASE}/business/get/?${params}`, { headers: hdrs(tokens) });
  return parseRes(res);
}

// ── Videos (requires TikTok account token) ────────────────────────────────────

export async function listVideos({ cursor = 0, pageSize = 20 } = {}) {
  const tokens = await acctAuth();
  const fields = JSON.stringify(['item_id', 'caption', 'create_time', 'thumbnail_url', 'video_views', 'likes', 'comments', 'shares', 'reach']);
  const params = new URLSearchParams({ business_id: tokens.business_id, cursor, page_size: pageSize, fields });
