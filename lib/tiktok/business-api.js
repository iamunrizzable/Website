...
// TikTok action endpoints expect comment_id/video_id as int64 (unquoted JSON integers).
// JS can't represent 19-digit integers precisely, so we build the body as a string and
// unquote any large digit strings to restore them as proper JSON integers.
function bodyWithInts(obj) {
  return JSON.stringify(obj).replace(/"(\d{16,})"/g, '$1');
}
...
export async function hideComment({ commentId, isHidden = true, videoId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/hide/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: bodyWithInts({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId, is_hidden: isHidden }),
  });
  return parseRes(res);
}
...
export async function replyToComment({ videoId, commentId, content }) {
  ...
  const res = await fetch(`${BASE}/business/comment/reply/create/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: bodyWithInts({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId, text: content }),
  });
  return parseRes(res);
}
...
export async function deleteComment({ commentId, videoId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/delete/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: bodyWithInts({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId }),
  });
  return parseRes(res);
}
...
export async function pinComment({ commentId, isPinned = true, videoId }) {
  ...
  const res = await fetch(`${BASE}/business/comment/pin/`, {
    method: 'POST',
    headers: hdrs(tokens),
    body: bodyWithInts({ business_id: tokens.business_id, video_id: videoId, comment_id: commentId, is_pinned: isPinned }),
  });
  return parseRes(res);
}
