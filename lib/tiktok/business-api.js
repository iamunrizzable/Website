...
export async function listComments({ videoId, cursor = 0, pageSize = 20 }) {
  ...
  const businessId = process.env.TIKTOK_BUSINESS_ID ?? biz?.open_id ?? tokens.business_id;
...
async function commentAction(endpoint, payload) {
  const biz = await auth(); // advertiser token — has permission for comment write ops
  const businessId = process.env.TIKTOK_BUSINESS_ID ?? biz?.open_id;
  const body = actionBody({ business_id: businessId, ...payload });
  const res = await fetch(`${BASE}${endpoint}`, { method: 'POST', headers: hdrs(biz), body });
