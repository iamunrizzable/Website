...
// Refreshes an account token via TikTok's documented Open API OAuth refresh
// (developers.tiktok.com/doc/oauth-user-access-token-management) — confirmed
// primary-source endpoint and payload shape, unlike the advertiser-token
// refresh. Response fields are access_token, expires_in, open_id,
// refresh_expires_in, refresh_token, scope, token_type — notably NO
// business_id, so callers must preserve the previously known business_id
// themselves (this endpoint can't confirm whether a Business-Portal-issued
// token's refresh_token is valid here too; a rejection just means that
// account should fall back to the existing "reconnect" flow).
export async function refreshTikTokAccountToken(refreshToken) {
  const res = await fetch('https://open.tiktokapis.com/v2/oauth/token/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
