...
async function recordVisitorHistory(visitorId, { ip, location, userAgent } = {}) {
  if (!visitorId) return;
  try {
    const now = new Date().toISOString();
    const redis = getRedis();
    if (redis) {
      const existingRaw = await redis.hget(VISITOR_HISTORY_KEY, visitorId);
      ...
      await redis.hset(VISITOR_HISTORY_KEY, { [visitorId]: JSON.stringify(record) });
      try {
        ...
        await redis.hexpire(VISITOR_HISTORY_KEY, visitorId, VISITOR_HISTORY_TTL_SECONDS);
      } catch {
        // Non-fatal.
      }
    } else {
...
export async function getVisitorHistory(visitorId) {
  ...
  try {
    const redis = getRedis();
    if (redis) {
      const raw = await redis.hget(VISITOR_HISTORY_KEY, visitorId);
      return raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
    }
    return mem.get(VISITOR_HISTORY_KEY)?.get(visitorId) ?? null;
  } catch {
    return null;
  }
}
...
// near-misses. Sorted newest-lastSeen-first, the ordering that page shows.
export async function getAllVisitorHistory() {
  try {
    ...
    const visitors = Object.entries(raw).map(([visitorId, value]) => ({
      visitorId,
...
export async function checkDeviceAgainstBlocklist({
  ...
  await recordVisitorHistory(visitorId, { ip, location, userAgent });
