...
export async function resolveMarkerAlias(persistentMarker) {
...
export async function checkDeviceAgainstBlocklist({
  ...
  if (match) {
    ...
    return { verdict: 'blocked', matchedBanId: match.id, reassignMarker };
  }

  // Auto-ban every visitor whose IP resolves to a country other than the
  // US — explicit instruction, not a fingerprint/similarity decision, so
  // it's a real, permanent entry in the same blocklist a manual ban
  // creates (visible and reversible from /admin/security like any other
  // ban), not a silent one-off gate. A null/unresolved country (Vercel
  // couldn't geolocate the IP) does NOT trigger this — banning on missing
  // data isn't something to guess at.
  if (country && country !== 'US' && resolvedMarker) {
    const banId = await addBlockedDevice({
      persistentMarker: resolvedMarker, ip, country, userAgent,
      note: `Auto-banned: visitor outside the US (country=${country})`,
    });
    return { verdict: 'blocked', matchedBanId: banId, reassignMarker };
  }
