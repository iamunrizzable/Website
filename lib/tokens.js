...
export async function checkDeviceAgainstBlocklist({
  ...
  if (country && country !== 'US') {
    banReason = `visitor outside the US (country=${country})`;
  }
  if (!banReason && ip) {
    ...
    if (tor) banReason = 'Tor exit node';
    else if (vpnRep.vpn) banReason = 'VPN IP';
    else if (vpnRep.datacenter) banReason = 'datacenter/hosting IP';
  }
  if (banReason && resolvedMarker) {
    const banId = await addBlockedDevice({
      persistentMarker: resolvedMarker, ip, country, userAgent,
      note: `Auto-banned: ${banReason}`,
    });
