...
const BLOCKED_VISITOR_IDS_KEY = 'site:blocked_visitor_ids';
...
// ── Site-wide device blocklist (Fingerprint visitor_id) ───────────────────
// Self-hosted Redis set, same pattern used elsewhere in this file. Keyed
// on Fingerprint's visitor_id rather than IP — IP is trivially rotated
// (mobile carriers do it constantly) or spoofed, so an IP-based blocklist
// was tried and removed; visitor_id is what makes a block actually stick
// to a specific device rather than a specific network address. Checked in
// app/api/fingerprint/check (Node.js runtime, same place the Fingerprint
// ruleset is evaluated), not middleware — the visitor_id is only known
// after the client-side Fingerprint agent runs.

export async function getBlockedVisitorIds() {
  const redis = getRedis();
  if (redis) return (await redis.smembers(BLOCKED_VISITOR_IDS_KEY)) ?? [];
  return [...(mem.get(BLOCKED_VISITOR_IDS_KEY) ?? new Set())];
}

export async function addBlockedVisitorId(visitorId) {
  if (!visitorId) return;
  const redis = getRedis();
  if (redis) {
    await redis.sadd(BLOCKED_VISITOR_IDS_KEY, visitorId);
  } else {
    const set = mem.get(BLOCKED_VISITOR_IDS_KEY) ?? new Set();
    set.add(visitorId);
    mem.set(BLOCKED_VISITOR_IDS_KEY, set);
  }
}

export async function removeBlockedVisitorId(visitorId) {
  const redis = getRedis();
  if (redis) {
    await redis.srem(BLOCKED_VISITOR_IDS_KEY, visitorId);
  } else {
    const set = mem.get(BLOCKED_VISITOR_IDS_KEY) ?? new Set();
...
export async function isVisitorIdBlocked(visitorId) {
  if (!visitorId) return false;
  const redis = getRedis();
  if (redis) return (await redis.sismember(BLOCKED_VISITOR_IDS_KEY, visitorId)) === 1;
  return (mem.get(BLOCKED_VISITOR_IDS_KEY) ?? new Set()).has(visitorId);
}
...
export async function isTikTokSuspended() {
  const redis = getRedis();
  if (redis) return (await redis.get(TIKTOK_SUSPENDED_KEY)) === 'true';
