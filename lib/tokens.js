...
const VISITOR_HISTORY_TTL_SECONDS = 90 * 24 * 60 * 60;
...
// plain `REDIS_URL` TCP connection string — confirmed directly from the
// value shown in the Vercel dashboard, not guessed — never the
// KV_REST_API_*/UPSTASH_REDIS_REST_* names @upstash/redis needs. That REST
// client can't speak `redis://` under any configuration, so REDIS_URL
...
export function getRedis() {
  ...
  if (process.env.REDIS_URL) return getTcpRedisAdapter();
  return null;
}
...
export function isRedisConfigured() {
  return !!getRedisEnv() || !!process.env.REDIS_URL;
}
...
// Runs on EVERY single check (lib/tokens.js's checkDeviceAgainstBlocklist,
// which gates the whole site via DeviceGate) — unlike every other Redis
// call in this file, this one had no error handling, so any Redis hiccup
// (a slow/failed connection, not just "unconfigured") threw uncovered all
// the way up to app/api/fingerprint/check/route.js's catch, which fails
// the WHOLE SITE closed for every visitor, not just the one check. That's
// the documented intent for a check we genuinely can't complete (see that
// route's comments) — but degrading to "no known bans" here, like the
// in-memory fallback already silently does on every cold start/redeploy
// (nothing new memory-wise), keeps a transient Redis issue from being a
// site-wide outage. A banned device could theoretically get back in during
// that window — already true of the mem-fallback path, not a new gap this
// introduces, and the honest tradeoff on the other side (blocking every
// real visitor) is worse for a moderation blocklist that isn't itself a
// security perimeter.
export async function getBlockedDevices() {
  try {
    ...
    console.error('[getBlockedDevices] Redis error, degrading to empty blocklist for this check:', err?.message ?? err);
    return [];
  }
}
...
async function recordVisitorHistory(visitorId, persistentMarker, { ip, location, userAgent } = {}) {
  ...
  try {
    ...
    const redis = getRedis();
    if (redis) {
      const existingRaw = await redis.hget(VISITOR_HISTORY_KEY, historyKey);
      ...
      try {
        // Per-field hash TTL (Redis 7.4+) — rolling 90-day expiry so a
      ...
      const existing = map.get(historyKey);
      map.set(historyKey, {
        visitorId: visitorId ?? existing?.visitorId ?? null,
        persistentMarker: persistentMarker ?? existing?.persistentMarker ?? null,
        firstSeenAt: existing?.firstSeenAt ?? now,
...
// Every visitor the site has ever fingerprinted (within the 90-day rolling
...
// lib/reputation/enrich.js) is computed ONLY on the near-miss branch —
// never on allowed/clean visits (nowhere to attach it) and never
// recomputed on a repeat hit against an already-banned device (see
// touchBlockedDevice above). It's informational only: nothing computed
...
export async function checkDeviceAgainstBlocklist({
  ...
  await recordVisitorHistory(visitorId, persistentMarker, { ip, location, userAgent });
