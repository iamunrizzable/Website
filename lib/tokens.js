...
//
// Keyed by persistentMarker — a 128-bit value generated once per device
...
export async function checkDeviceAgainstBlocklist({
  ...
  await recordVelocityEvent(persistentMarker, ip, country);
  await recordVisitorHistory(persistentMarker, { ip, location, userAgent, country, webgl, botSignals, privacySignals });

  const profiles = await getBlockedDevices();
  const match = persistentMarker ? profiles.find((p) => p.persistentMarker === persistentMarker) : null;
  if (match) {
    await touchBlockedDevice(match.id, location);
    return { verdict: 'blocked', matchedBanId: match.id };
  }
