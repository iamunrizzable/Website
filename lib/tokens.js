...
// Countries checkDeviceAgainstBlocklist's auto-ban leaves alone — everyone
// else gets permanently banned. Canada added alongside the US per explicit
// instruction (TikTok allows doing business with Canadian accounts).
...
export async function checkDeviceAgainstBlocklist({
  ...
  if (match) {
    ...
    // blockCountry is always this visit's live geolocation (not whatever
    // country the ban was originally created under), so it reflects where
    // they're actually connecting from right now.
    return { verdict: 'blocked', matchedBanId: match.id, reassignMarker, blockReason: match.reasonCode ?? null, blockCountry: country };
  }

  // Auto-ban: explicit instruction to block everyone outside the US and
  // Canada (TikTok allows business with Canadian accounts), closed against
  // the two realistic ways to fake being in an allowed country — a VPN or
  // Tor exit node whose own IP happens to geolocate as US/CA. A "blanket
  // VPN ban" only on disallowed-country traffic would miss exactly that
  // case, so Tor/VPN/datacenter detection runs regardless of what country
  // the IP reports, not only as a fallback when the country check doesn't
  // fire. Checked fresh on every visit — isTorExitNode/checkVpnReputation
  ...
  // exit node — indistinguishable from a genuine US/CA home connection
  // with IP-reputation lists alone. No IP-based signal closes that;
  // flagging it rather than claiming this is airtight.
  let reasonCode = null;
  let noteDetail = null;
  if (country && !ALLOWED_COUNTRIES.has(country)) {
    reasonCode = 'non-us';
    noteDetail = `visitor outside the US/Canada (country=${country})`;
  }
  if (!reasonCode && ip) {
  ...
  if (reasonCode && resolvedMarker) {
    ...
    return { verdict: 'blocked', matchedBanId: banId, reassignMarker, blockReason: reasonCode, blockCountry: country };
  }
